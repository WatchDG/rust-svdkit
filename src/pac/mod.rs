...
fn generate_memory_x(device: &svd::Device) -> String {
    ...
    out.writeln("/* AUTO-GENERATED BY svdkit::pac (memory layout) */")
        .unwrap();
    out.writeln(&format!("/* device: {} */", device.name))
        .unwrap();
    out.writeln(&format!(
        "/* address_unit_bits: {} */",
        device.address_unit_bits
    ))
    .unwrap();
    out.writeln(&format!("/* width: {} */", device.width))
        .unwrap();
    out.writeln("").unwrap();
    ...
    let (flash_origin, flash_size, ram_origin, ram_size) = {
        if !buffer_ranges.is_empty() {
            ...
            out.writeln(&format!(
                "/* Found {} buffer-type address block(s): */",
                buffer_ranges.len()
            ))
            .unwrap();
            for (s, e, name) in &buffer_ranges {
                out.writeln(&format!(
                    "/*   {name}: 0x{s:08X} - 0x{e:08X} ({} KiB) */",
                    (e - s) / 1024
                ))
                .unwrap();
            }
            ...
            all_reg_ranges.sort_by_key(|r| r.0);
            out.writeln("/* No buffer-type address blocks found. */")
                .unwrap();
            out.writeln(
                "/* Using heuristics: FLASH from 0 to first peripheral, RAM from 0x20000000. */",
            )
            .unwrap();
            out.writeln("/* VERIFY these values against the datasheet! */")
                .unwrap();
            out.writeln("").unwrap();
            ...
            out.writeln("/* No address blocks found. Using default Cortex-M layout. */")
                .unwrap();
            out.writeln("/* VERIFY these values against the datasheet! */")
                .unwrap();
            out.writeln("").unwrap();
    ...
    out.writeln(&format!("FLASH_ORIGIN = 0x{flash_origin:08X};"))
        .unwrap();
    out.writeln(&format!("FLASH_SIZE   = 0x{flash_size:08X};"))
        .unwrap();
    out.writeln("").unwrap();
    out.writeln(&format!("RAM_ORIGIN = 0x{ram_origin:08X};"))
        .unwrap();
    out.writeln(&format!("RAM_SIZE   = 0x{ram_size:08X};"))
        .unwrap();
    out.writeln("").unwrap();
    ...
    out.writeln("{").unwrap();
    out.indent();
    out.writeln("FLASH (rx) : ORIGIN = FLASH_ORIGIN, LENGTH = FLASH_SIZE")
        .unwrap();
    out.writeln("RAM  (rwx) : ORIGIN = RAM_ORIGIN,  LENGTH = RAM_SIZE")
        .unwrap();
    out.dedent();
    out.writeln("}").unwrap();
    out.writeln("").unwrap();

    out.writeln("__stack_size = DEFINED(__stack_size) ? __stack_size : 0x400;")
        .unwrap();
    out.writeln("").unwrap();
    out.writeln("PROVIDE(__stack = ORIGIN(RAM) + LENGTH(RAM));")
        .unwrap();
