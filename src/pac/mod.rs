...
pub fn generate_device_files_with_rt(device: &svd::Device) -> Result<Vec<GeneratedFile>> {
    ...
    out.push(GeneratedFile {
        file_name: "memory.x".to_string(),
        content: generate_memory_x(device),
    });
    Ok(out)
}

/// Записывает `<device>_pac/` + `<device>_rt.rs` + `<device>_link.x` + `memory.x` в `out_dir`.
...
fn sanitize_type_name(s: &str) -> String {
    ...
    if out.is_empty() {
        ...
        out.replace("Ldetect", "LDetect")
            ...
            .replace("Constlat", "ConstLat")
            .replace("Lowpwr", "LowPwr")
            .replace("Pofwarn", "PofWarn")
            .replace("Sleepenter", "SleepEnter")
            .replace("Sleepexit", "SleepExit")
            .replace("Usbdetected", "UsbDetected")
            .replace("Usbremoved", "UsbRemoved")
            .replace("Usbpwrrdy", "UsbPwrRdy")
            .replace("Ramstatus", "RamStatus")
            .replace("Usbregstatus", "UsbRegStatus")
            .replace("Mainregstatus", "MainRegStatus")
            .replace("MainRegStatusMainRegStatus", "MainRegStatus")
            .replace("Dcdcen", "DcDcEn")
            .replace("DcDcEnDcDcEn", "DcDcEn")
            .replace("DcDcEn0DcDcEn", "DcDcEn0")
            .replace("EventsPofWarnEventsPofWarn", "EventsPofWarn")
            .replace("TasksConstLatTasksConstLat", "TasksConstLat")
            .replace("TasksLowPwrTasksLowPwr", "TasksLowPwr")
            .replace("EventsSleepEnterEventsSleepEnter", "EventsSleepEnter")
            .replace("EventsSleepExitEventsSleepExit", "EventsSleepExit")
            .replace("EventsUsbDetectedEventsUsbDetected", "EventsUsbDetected")
            .replace("EventsUsbRemovedEventsUsbRemoved", "EventsUsbRemoved")
            .replace("EventsUsbPwrRdyEventsUsbPwrRdy", "EventsUsbPwrRdy")
...
fn generate_memory_x(device: &svd::Device) -> String {
    let mut out = CodeWriter::new();

    out.writeln("/* AUTO-GENERATED BY svdkit::pac (memory layout) */").unwrap();
    out.writeln(&format!("/* device: {} */", device.name)).unwrap();
    out.writeln(&format!("/* address_unit_bits: {} */", device.address_unit_bits)).unwrap();
    out.writeln(&format!("/* width: {} */", device.width)).unwrap();
    out.writeln("").unwrap();

    let mut buffer_ranges: Vec<(u64, u64, &str)> = Vec::new();
    let mut all_reg_ranges: Vec<(u64, u64)> = Vec::new();

    for p in &device.peripherals {
        for ab in &p.address_block {
            let start = p.base_address.saturating_add(ab.offset);
            let size = ab.size;
            if size == 0 {
                continue;
            }
            ...
            match ab.usage {
                svd::AddressBlockUsage::Buffer => {
                    buffer_ranges.push((start, end, p.name.as_str()));
                }
                svd::AddressBlockUsage::Registers => {
                    all_reg_ranges.push((start, end));
                }
                svd::AddressBlockUsage::Reserved => {
                    all_reg_ranges.push((start, end));
                }
            }
        }
    ...
    if !all_reg_ranges.is_empty() {
        all_reg_ranges.sort_by_key(|r| r.0);
    }

    let (flash_origin, flash_size, ram_origin, ram_size) = {
        if !buffer_ranges.is_empty() {
            buffer_ranges.sort_by_key(|r| r.0);
            let ram_start = buffer_ranges[0].0;
            let ram_end = buffer_ranges.last().map(|r| r.1).unwrap_or(ram_start);
            let ram_sz = ram_end.saturating_sub(ram_start);

            let flash_sz = if !all_reg_ranges.is_empty() {
                all_reg_ranges[0].0
            } else {
                0x100000
            };

            out.writeln(&format!("/* Found {} buffer-type address block(s): */", buffer_ranges.len())).unwrap();
            for (s, e, name) in &buffer_ranges {
                out.writeln(&format!("/*   {name}: 0x{s:08X} - 0x{e:08X} ({} KiB) */", (e - s) / 1024)).unwrap();
            }
            out.writeln("").unwrap();

            (0x00000000u64, flash_sz.max(0x1000), ram_start, ram_sz.max(0x1000))
        } else if !all_reg_ranges.is_empty() {
            all_reg_ranges.sort_by_key(|r| r.0);
            out.writeln("/* No buffer-type address blocks found. */").unwrap();
            out.writeln("/* Using heuristics: FLASH from 0 to first peripheral, RAM from 0x20000000. */").unwrap();
            out.writeln("/* VERIFY these values against the datasheet! */").unwrap();
            out.writeln("").unwrap();
            let first_reg = all_reg_ranges[0].0;
            let last_reg = all_reg_ranges.last().map(|r| r.1).unwrap_or(0);
            let flash_sz = first_reg.max(0x100000);
            let ram_start = last_reg.max(0x20000000);
            let ram_sz = 0x40000u64;
            (0x00000000u64, flash_sz, ram_start, ram_sz)
        } else {
            out.writeln("/* No address blocks found. Using default Cortex-M layout. */").unwrap();
            out.writeln("/* VERIFY these values against the datasheet! */").unwrap();
            out.writeln("").unwrap();
            (0x00000000u64, 0x100000u64, 0x20000000u64, 0x40000u64)
        }
    };

    out.writeln(&format!("FLASH_ORIGIN = 0x{flash_origin:08X};")).unwrap();
    out.writeln(&format!("FLASH_SIZE   = 0x{flash_size:08X};")).unwrap();
    out.writeln("").unwrap();
    out.writeln(&format!("RAM_ORIGIN = 0x{ram_origin:08X};")).unwrap();
    out.writeln(&format!("RAM_SIZE   = 0x{ram_size:08X};")).unwrap();
    out.writeln("").unwrap();

    out.writeln("MEMORY").unwrap();
    out.writeln("{").unwrap();
    out.indent();
    out.writeln("FLASH (rx) : ORIGIN = FLASH_ORIGIN, LENGTH = FLASH_SIZE").unwrap();
    out.writeln("RAM  (rwx) : ORIGIN = RAM_ORIGIN,  LENGTH = RAM_SIZE").unwrap();
    out.dedent();
    out.writeln("}").unwrap();
    out.writeln("").unwrap();

    out.writeln("__stack_size = DEFINED(__stack_size) ? __stack_size : 0x400;").unwrap();
    out.writeln("").unwrap();
    out.writeln("PROVIDE(__stack = ORIGIN(RAM) + LENGTH(RAM));").unwrap();
