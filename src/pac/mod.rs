...
fn generate_rt_rs(device: &svd::Device) -> Result<String> {
    ...
    out.writeln("//")?;
    out.writeln("// Minimal Cortex-M runtime without cortex-m-rt:")?;
    out.writeln("// - vector table")?;
    out.writeln("// - Reset handler")?;
    out.writeln("// - .data/.bss init")?;
    out.writeln("//")?;
    out.writeln("#![allow(non_snake_case)]")?;
    out.writeln("#![allow(dead_code)]")?;
    out.writeln("")?;
    ...
    out.indent();
    out.writeln("// Set VTOR to this image's vector table (if supported by the core).")?;
    out.writeln("let vtor = 0xE000_ED08usize as *mut u32;")?;
    out.writeln("ptr::write_volatile(vtor, ptr::addr_of!(__vector_table) as u32);")?;
    out.writeln("")?;
    out.writeln("// Copy .data from FLASH (LMA) to RAM (VMA).")?;
    ...
    // Vector table pieces (cortex-m-rt compatible section names).
    out.writeln("#[used]")?;
