    ...
    for it in sorted {
        let (name, off, size_bytes, kind_desc) = match it {
            svd::RegisterBlockItem::Register { register } => {
                let (_, elem_sz) = reg_rust_type_and_size(ctx, register);
        ...
        match it {
            svd::RegisterBlockItem::Register { register } => {
                emit_register_field(out, ctx, &name, register)?;
            }
            svd::RegisterBlockItem::Cluster { cluster } => {
...
fn emit_register_field(
    ...
    if let Some(dim) = &r.dim {
        ...
        out.writeln(&format!("pub {field_name}: {ty},"))?;
    }
    Ok(())
}
...
fn reg_rust_type_and_size(ctx: &Ctx<'_>, r: &svd::Register) -> (String, u64) {
    let (base_ty, bytes) = reg_primitive_ty_and_size(ctx, &r.properties);
